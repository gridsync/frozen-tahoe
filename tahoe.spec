...
# Disable runtime version checks for some packages. Needed for frozen builds.
try:
    ...
    with open(auto_deps) as f, open('_auto_deps.py.modified', 'w+') as n:
        n.write(f.read()
            ...
            .replace('"magic-wormhole >=', '#"magic-wormhole >='))
...
a = Analysis(['static/tahoe.py'],
             ...
             binaries=None,
             datas=added_files,
             hiddenimports=['characteristic', 'cffi', 'Crypto', 'yaml', 'zfec'],
             hookspath=[],
             runtime_hooks=[],
             excludes=[],
